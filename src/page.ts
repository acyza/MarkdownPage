...
export async function loadPage(config: ConfigTemplate, axios: Axios) {
  ...
  async function analysis(
    ...
    const leftRegExp = /\{\{[^{]/g,
      rightRegExp = /[^}]\}\}/g;
    while (leftRegExp.exec(data)) {
      result.push(
        data.substring(rightRegExp.lastIndex, leftRegExp.lastIndex - 3)
      );
      ...
      result.push(
        await get(
          data
            .substring(leftRegExp.lastIndex - 1, rightRegExp.lastIndex - 2)
            .trim()
        )
      );
    }
  ...
  document.body.innerHTML = await analysis(
    config.template ? await get(config.template) : defaultTemplate,
    get
  );
}
