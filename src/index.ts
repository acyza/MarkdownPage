...
function mathExtension(
  level: "block" | "inline"
): _marked.MarkedExtension["extensions"] extends (infer R)[] ? R : undefined {
  const result = {
    name: `math_${level}`,
    level: level,
    ...
    renderer: (token) =>
      `<p class=${result.name}>${katex.renderToString(token.text, {
        throwOnError: false,
      })}</p>`,
  };
  return result;
...
marked.use({
  ...
  renderer: {
    ...
    image(href, title, text) {
      return href === null
        ? text
        : `<img src="${util.fillPath(
            href,
            util.folder("./doc/" + mlp.current)
          )}"
          ${title ? `title=${title}` : ""}
          />`;
    },
...
marked.setOptions({
  highlight(code, lang, callback) {
    ...
    if (callback) callback(result.errorRaised, result.value);
    return result.value;
  },
});
...
const requestMarkdown = (() => {
  ...
  const callback: { success: (data) => void; error?: (err) => void } = {
    success: undefined,
  };
...
function go(path: string) {
  ...
  const position = /.*#.*/.test(path) ? path.replace(/.*#/, "") : "";
...
let title: HTMLTitleElement | undefined | null,
  content: HTMLElement | Element | undefined | null;
...
export function load() {
  ...
  let _title = (title.innerText = mlp.title);
