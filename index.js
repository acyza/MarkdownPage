...
function getProcessArgs() {
  ...
  for (let i = 2; i < args.length; i++) {
    if (/^-/.test(args[i])) result[args[i].replace(/^-/, "")] ||= true;
...
function margeConfig(dist, src) {
  for (let key in dist) if (src[key]) dist[key] = src[key];
}

async function configInit() {
  if (existsSync("mlp-config.json")) {
    const _config = JSON.parse((await readFile("mlp-config.json")).toString());
...
async function isEmptyFolder(url) {
  ...
  for await (const f of dir) return false;
  return true;
}
...
const gen = async () => {
  if (!existsSync(config.doc) || (await isEmptyFolder(config.doc)))
    return console.log(
      "Please put the markdown document in the .//doc folder or config document path"
    );
  else {
...
async function main() {
  ...
  if (processArgs["perview"]) {
    const server = new http.Server(async (req, res) => {
      if (
        !new RegExp(`^${process.cwd}/dist/.*`).test(path.normalize(req.url))
      ) {
        req.statusCode = 500;
        res.write("error");
        return res.end();
